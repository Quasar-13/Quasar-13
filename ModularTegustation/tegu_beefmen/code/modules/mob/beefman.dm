...
/datum/species/beefman/disarm(mob/living/carbon/human/user, mob/living/carbon/human/target, datum/martial_art/attacker_style)
	...
	if (user == target)
		...
		if ((target_zone in allowedList) && affecting)
			...
			playsound(get_turf(user), 'ModularTegustation/tegu_beefmen/sounds/beef_hit.ogg', 40, 1)
		...
		if ((target_zone in allowedList))
			...
			spawn(1)
				if (do_mob(user,H))
					...
					newBP.attach_limb(H)
					newBP.give_meat(H, I)
					playsound(get_turf(H), 'ModularTegustation/tegu_beefmen/sounds/beef_grab.ogg', 50, 1)
...
/obj/item/bodypart/head/beef
	icon = 'ModularTegustation/tegu_beefmen/icons/mob/beefman_bodyparts.dmi'
	icon_greyscale = 'ModularTegustation/tegu_beefmen/icons/mob/beefman_bodyparts.dmi'
	icon_greyscale_robotic = 'ModularTegustation/tegu_beefmen/icons/mob/beefman_bodyparts_robotic.dmi'
