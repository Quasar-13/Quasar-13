...
/*
//These are previously pasted IAA code, don't know if I need it so putting it here for reference just in case
/datum/game_mode/traitor/nt_blackops/post_setup()
	var/ntop = 0
	for(var/datum/mind/traitor in pre_traitors)
		ntop++
		if(ntop + 1 > pre_traitors.len)
			ntop = 0
...
/datum/game_mode/traitor/nt_blackops/add_latejoin_traitor(datum/mind/character)
	...
	if(late_joining_list.len >= rand(3, 4))
		...
		for(var/datum/mind/traitor in late_joining_list)
			ntop++
			if(ntop + 1 > late_joining_list.len)
				ntop = 0
