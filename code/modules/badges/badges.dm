...
Special ranks:
- Trusted: Matches clients in the trusted_players list
...
/client/proc/get_badges()
	...
	//Add the mentor rank, if user is not an admin
	else if(mentor_datum && GLOB.badge_data["Mentor"])
		badges += GLOB.badge_data["Mentor"]

	//Add the trusted player rank
	if(is_trusted_player(ckey) && GLOB.badge_data["Trusted"])
		badges += GLOB.badge_data["Trusted"]
