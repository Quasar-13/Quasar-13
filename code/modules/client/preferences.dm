	...
	if(href_list["preference"] == "job")
		switch(href_list["task"])
			...
			if("alt_title")
				...
				if(user.client.prefs.exp[job_title] > (J.get_exp_req_amount() + GET_CONFIG(number/senior_timelock))) //If they have more than 50 hours (300 Minutes) past the required time needed for the job, give them access to the senior title
				if(J.senior_title)
