...
/proc/set_security_level(level)
	...
	if(level >= SEC_LEVEL_GREEN && level <= SEC_LEVEL_DELTA && level != GLOB.security_level)
		...
		switch(level)
			...
			if(SEC_LEVEL_GAMMA)
				minor_announce(CONFIG_GET(string/alert_gamma), "Attention! Gamma security level activated!", sound = 'sound/misc/gamma_alert.ogg', TRUE)
				if(GLOB.security_level == SEC_LEVEL_GREEN)
					modTimer = 0.25
				else
					modTimer = 0.5
			...
			if(SEC_LEVEL_EPSILON)
				minor_announce(CONFIG_GET(string/alert_epsilon), "Attention! Epsilon security level reached!", sound = 'sound/misc/epsilon_alert.ogg', TRUE)
