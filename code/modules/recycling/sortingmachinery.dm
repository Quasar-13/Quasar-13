...
/obj/structure/big_delivery/attackby(obj/item/W, mob/user, params)
	if(istype(W, /obj/item/dest_tagger))
		...
		if(sortTag != O.currTag)
			...
			to_chat(user, spantext("SELECTED DESTINATION: [tag]"))
			sortTag = O.currTag
...
/obj/item/dest_tagger/suicide_act(mob/living/user)
	...
	if (islizard(user))
		to_chat(user, spantext("SELECTED DESTINATION: HELL"))//lizard nerf
	else
		to_chat(user, spantext("SELECTED DESTINATION: HEAVEN"))
