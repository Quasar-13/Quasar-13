...
*/
/n_Interpreter/proc/RaiseError(runtimeError/e, scope/scope, token/token)
	e.scope = scope
	if(istype(token))
		e.token = token
...
*/
/n_Interpreter/proc/AlertAdmins()
...
*/
/n_Interpreter/proc/RunBlock(node/BlockDefinition/Block, scope/scope = globalScope)
...
*/
/n_Interpreter/proc/RunFunction(node/expression/FunctionCall/stmt, scope/scope)
...
*/
/n_Interpreter/proc/RunIf(node/statement/IfStatement/stmt, scope/scope)
	if(!stmt.skip)
...
/*
	...
	Runs a while loop.
*/
/n_Interpreter/proc/RunWhile(node/statement/WhileLoop/stmt, scope/scope)
	var/i=1
...
/n_Interpreter/proc/RunFor(node/statement/ForLoop/stmt, scope/scope)
	var/i=1
	scope = scope.push(stmt.block)
...
*/
/n_Interpreter/proc/Iterate(node/BlockDefinition/block, scope/scope, count)
