...
/mob/living/simple_animal/hostile/megafauna/apostle
	...
	icon = 'ModularBungalow/legacy_icons/64x64.dmi'
	icon_state = "angel"
	icon_living = "angel"
	...
	deathsound = 'ModularBungalow/zbungalowsounds/apostle/mob/apostle_death.ogg'
...
/datum/action/small_sprite/megafauna/tegu
	small_icon = 'ModularBungalow/legacy_icons/megafauna.dmi'
...
/mob/living/simple_animal/hostile/megafauna/apostle/proc/revive_humans()
	...
	playsound(src, 'ModularBungalow/zbungalowsounds/apostle/mob/apostle_spell.ogg', 150, 1)
			...
			if(!("apostle" in H.faction))
				if(apostle_num < 13 && H.stat == DEAD && apostle_cooldown <= world.time && H.mind)
					...
					if(apostle_num < 12)
						...
						var/mutable_appearance/apostle_halo = mutable_appearance('ModularBungalow/legacy_icons/32x64.dmi', "halo", -HALO_LAYER)
					...
					for(var/mob/M in GLOB.player_list)
						if(M.z == z && M.client)
							...
							SEND_SOUND(M, 'ModularBungalow/zbungalowsounds/apostle/mob/apostle_bell.ogg')
...
/mob/living/simple_animal/hostile/megafauna/apostle/proc/deafening_scream()
	...
	playsound(src, 'ModularBungalow/zbungalowsounds/apostle/mob/apostle_shout.ogg', 70, 1)
...
/mob/living/simple_animal/hostile/megafauna/apostle/proc/rapture()
	...
	for(var/mob/M in GLOB.player_list)
		if(M.client) // Send it to every player currently active, not just everyone on Z-level
			SEND_SOUND(M, 'ModularBungalow/zbungalowsounds/apostle/antagonist/rapture.ogg')
	...
	for(var/mob/M in GLOB.player_list)
		if(M.z == z && M.client)
			SEND_SOUND(M, 'ModularBungalow/zbungalowsounds/apostle/antagonist/rapture2.ogg')
