...
/obj/item/kitchen/knife/letter_opener
    name = "letter opener"
    icon = 'ModularBungalow/legacy_icons/kirie_stuff/kirieweapons.dmi'
...
/obj/item/melee/nano_blade
	...
	var/dash_sound = 'ModularBungalow/zbungalowsounds/weapons/unsheathed_blade.ogg'
...
/obj/item/melee/nano_blade/proc/primed_attack(turf/target, mob/living/user)
	...
	for(var/turf/T in getline(start, get_turf(target)))
		for(var/mob/living/victim in T)
			if(victim != user)
				playsound(victim, 'ModularBungalow/zbungalowsounds/weapons/anime_slash.ogg', 30, TRUE)
...
/obj/item/storage/belt/nano_blade/AltClick(mob/user)
	...
	if(length(contents))
		var/obj/item/I = contents[1]
		playsound(user, 'ModularBungalow/zbungalowsounds/weapons/unsheathed_blade.ogg', 25, TRUE)
